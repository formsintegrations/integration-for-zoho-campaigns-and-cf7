...
export default function ZohoCampaignsAuthorization({ formID, campaignsConf, setCampaignsConf, step, setstep, isLoading, setIsLoading, setSnackbar, redirectLocation, isInfo }) {
  ...
  return (
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
      <div className="mt-3"><b>{__('Integration Name:', 'fitzocacf')}</b></div>
      <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="name" value={campaignsConf.name} type="text" placeholder={__('Integration Name...', 'fitzocacf')} disabled={isInfo} />

      <div className="mt-3"><b>{__('Data Center:', 'fitzocacf')}</b></div>
      <select onChange={handleInput} name="dataCenter" value={campaignsConf.dataCenter} className="btcd-paper-inp w-6 mt-1" disabled={isInfo}>
        <option value="">{__('--Select a data center--', 'fitzocacf')}</option>
      ...
      <div className="mt-3"><b>{__('Homepage URL:', 'fitzocacf')}</b></div>
      <CopyText value={`${window.location.origin}`} className="field-key-cpy w-6 ml-0" setSnackbar={setSnackbar} readOnly={isInfo} />

      <div className="mt-3"><b>{__('Authorized Redirect URIs:', 'fitzocacf')}</b></div>
      ...
      <small className="d-blk mt-5">
        {__('To get Client ID and SECRET , Please Visit', 'fitzocacf')}
        {' '}
        <a className="btcd-link" href={`https://api-console.zoho.${campaignsConf?.dataCenter || 'com'}/`} target="_blank" rel="noreferrer">{__('Zoho API Console', 'fitzocacf')}</a>
      </small>

      <div className="mt-3"><b>{__('Client id:', 'fitzocacf')}</b></div>
      <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="clientId" value={campaignsConf.clientId} type="text" placeholder={__('Client id...', 'fitzocacf')} disabled={isInfo} />
      <div style={{ color: 'red' }}>{error.clientId}</div>

      <div className="mt-3"><b>{__('Client secret:', 'fitzocacf')}</b></div>
      <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="clientSecret" value={campaignsConf.clientSecret} type="text" placeholder={__('Client secret...', 'fitzocacf')} disabled={isInfo} />
      ...
      {!isInfo && (
        <>
          <button onClick={() => handleAuthorize('zohoCampaigns', 'zcampaigns', scopes, campaignsConf, setCampaignsConf, setError, setisAuthorized, setIsLoading, setSnackbar, fitzocacf)} className="btn btcd-btn-lg green sh-sm flx" type="button" disabled={isAuthorized || isLoading}>
            {isAuthorized ? __('Authorized ✔', 'fitzocacf') : __('Authorize', 'fitzocacf')}
