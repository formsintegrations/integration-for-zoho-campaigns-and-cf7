  ...
  bitsFetch(refreshListsRequestParams, 'zcampaigns_refresh_lists')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: __('Lists refreshed', 'fitzocacf') })
        setCampaignsConf({ ...newConf })
      } else if ((result && result.data && result.data.data) || (!result.success && typeof result.data === 'string')) {
        setSnackbar({ show: true, msg: `${__('Lists refresh failed Cause:', 'fitzocacf')}${result.data.data || result.data}. ${__('please try again', 'fitzocacf')}` })
      } else {
        setSnackbar({ show: true, msg: __('Lists refresh failed. please try again', 'fitzocacf') })
      }
      setIsLoading(false)
    })
    ...
    .then(result => {
      if (result && result.success) {
        ...
        if (result.data.fields) {
          ...
          setSnackbar({ show: true, msg: __('Contact Fields refreshed', 'fitzocacf') })
        } else {
          setSnackbar({ show: true, msg: __('Zoho didn\'t provide fields names for this list', 'fitzocacf') })
        }
        ...
        setSnackbar({ show: true, msg: __('Contact Fields refresh failed. please try again', 'fitzocacf') })
      }
      setIsLoading(false)
    })
    .catch(() => setIsLoading(false))
