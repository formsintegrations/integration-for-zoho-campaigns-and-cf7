  ...
  return (
    ...
    >
      <select className="btcd-paper-inp mr-2" name="formField" value={field.formField || ''} onChange={(ev) => handleFieldMapping(ev, i, campaignsConf, setCampaignsConf)}>
        <option value="">{__('Select Field', 'fitzocacf')}</option>
        <optgroup label="Form Fields">
          {
            formFields.map(f => f.type !== 'file' && <option key={`ff-zhcrm-${f.name}`} value={f.name}>{f.label}</option>)
          }
        </optgroup>
        <option value="custom">{__('Custom...', 'fitzocacf')}</option>
      ...
      {field.formField === 'custom' && <TagifyInput onChange={e => handleCustomValue(e, i, campaignsConf, setCampaignsConf)} label={__('Custom Value', 'fitzocacf')} className="mr-2" type="text" value={field.customValue} placeholder={__('Custom Value', 'fitzocacf')} formFields={formFields} />}
