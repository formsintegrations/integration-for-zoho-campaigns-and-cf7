...
export default function ZohoCampaignsIntegLayout({ formID, formFields, handleInput, campaignsConf, setCampaignsConf, isLoading, setIsLoading, setSnackbar }) {
  return (
    <>
      ...
      <button onClick={() => refreshLists(formID, campaignsConf, setCampaignsConf, setIsLoading, setSnackbar)} className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh Campaigns Lists', 'fitzocacf')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
      <br />
      <br />
      {isLoading && (
        <Loader style={{
      ...
      {campaignsConf.list && (
        <>
          <div className="mt-4">
            <b className="wdt-100">{__('Map Fields', 'fitzocacf')}</b>
            <button onClick={() => refreshContactFields(formID, campaignsConf, setCampaignsConf, setIsLoading, setSnackbar)} className="icn-btn sh-sm ml-2 mr-2 tooltip" style={{ '--tooltip-txt': `'${__('Refresh Campaigns Contact Fields', 'fitzocacf')}'` }} type="button" disabled={isLoading}>&#x21BB;</button>
          </div>
            ...
            && (
              <>
                <div className="flx flx-around mt-2 mb-2 fitzocacf-field-map-label">
                  <div className="txt-dp"><b>{__('Form Fields', 'fitzocacf')}</b></div>
                  <div className="txt-dp"><b>{__('Zoho Fields', 'fitzocacf')}</b></div>
                </div>
