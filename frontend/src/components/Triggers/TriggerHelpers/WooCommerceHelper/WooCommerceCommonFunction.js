...
export const getAllOrderStatus = (data, setFlow) => {
  ...
  toast.promise(loadPostTypes, {
    success: (data) => data,
    error: __('Error Occurred', 'fitzocacf'),
...
export const getVariationsByProduct = (val, tmpNewFlow, setNewFlow, edit = false) => {
  ...
  const loadPostTypes = bitsFetch(null, 'get_all_variation_by_product', queryParams, 'GET').then((result) => {
    if (result && result.success) {
      // rubel vai code with immer js
      setNewFlow(produce(tmpNewFlow, draftConf => {
        if (!edit) {
  ...
  toast.promise(loadPostTypes, {
    success: (data) => data,
    error: __('Error Occurred here', 'fitzocacf'),
