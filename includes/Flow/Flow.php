...
final class Flow
{
    ...
    public function save($data)
    {
        ...
        wp_send_json_success(['id' => $saveStatus, 'msg' => __('Integration saved successfully', 'fitzocacf')]);
    }
    ...
    public function flowClone($data)
    {
        ...
        if (!\is_null($missingId)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'fitzocacf'), $missingId));
        }
        $integrationHandler = new FlowController();
        $integrations       = $integrationHandler->get(
            ['id' => $data->id],
        ...
        if (!is_wp_error($integrations) && \count($integrations) > 0) {
            ...
            wp_send_json_error(__('Flow ID is not exists', 'fitzocacf'));
        }
    }
    ...
    public function update($data)
    {
        ...
        wp_send_json_success(__('Integration updated successfully', 'fitzocacf'));
    }
    ...
    public function delete($data)
    {
        ...
        wp_send_json_success(__('Integration deleted successfully', 'fitzocacf'));
    }

    public function bulkDelete($param)
    {
        if (!\is_array($param->flowID) || $param->flowID === []) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'fitzocacf'), 'Integration id'));
        }
    ...
    public function toggle_status($data)
    {
        ...
        wp_send_json_success(__('Status changed successfully', 'fitzocacf'));
    }
        ...
        if (!is_wp_error($flows) && !empty($flows)) {
            ...
            foreach ($flows as $flowData) {
                ...
                if (
                    ...
                    $error = new WP_Error('Conditional Logic False', __('Conditional Logic not matched', 'fitzocacf'));
